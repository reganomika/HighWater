...
Run this in a chat that's been open since before you last edited CLAUDE.md, so it picks up the current task-boundary rule without a restart. It re-reads `~/.claude/CLAUDE.md` and this project's own CLAUDE.md (if any), and returns one checkable artifact (the current `context-check.sh` thresholds, soft and hard, as a percentage of the window) so you can confirm it took effect instead of trusting a claim.
